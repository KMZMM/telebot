...
import requests
import asyncio  # Changed from time.sleep
import pyfiglet
...
async def handle_document(update: Update, context: CallbackContext) -> None:
    ...
    if processing_state['is_processing']:
        await update.message.reply_text("Already processing a file. Please wait.")
        return
        
    try:
        processing_state['is_processing'] = True
        processing_state['current_position'] = 0
...
async def process_combo(update: Update, context: CallbackContext, combo_file: str):
    ...
    with open(combo_file, 'r') as file:
        ...
        for i in range(processing_state['current_position'], total_lines):
            try:
                P = lines[i]
                processing_state['current_position'] = i + 1
                ...
                try:
                    ...
                    if 'Your card number is incorrect.' in r1.text:
                        await update.message.reply_text(f'[ {i+1} ] {P} ➠➠ Incorrect Number ❌')
                        continue
                    else:
...
✘ By ➠ KMZ BOT'''
                    ...
                    await update.message.reply_text(f'[ {i+1} ] {P} ➠➠ ✘ Card is Declined ')

            except Exception as e:
                await update.message.reply_text(f'Error processing line {i+1}: {str(e)}')
                continue
